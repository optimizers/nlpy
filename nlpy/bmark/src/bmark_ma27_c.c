...
int main( int argc, char **argv ) {
    ...
    for( k = 1; k < argc; k++ ) {
        ...
        /* Initialize */
        problem = Ma27_Initialize( nz, n, NULL );
        ...
        for( i = 0; i < problem->nz; i++ )
            fscanf( fp, "%d %d %lg\n",
                    &(problem->irn[i]), &(problem->icn[i]), &(a[i]) );
        ...
        /* rhs = A * e. Adjust indices to Fortran 1-based scheme */
        ...
        /* Analyze */
        error = Ma27_Analyze( problem, 0 );   // Automatic pivot sequence
        if( error ) {
            fprintf( stderr, "Error return from Ma27_Analyze: %-d\n", error );
            continue;
        }

        /* Factorize */
        error = Ma27_Factorize( problem, a );
        if( error ) {
            fprintf( stderr, "Error return from Ma27_Factorize: %-d\n", error );
            continue;
        }
        ...
        /* Solve linear system */
        error = Ma27_Solve( problem, rhs );
        if( error ) {
            fprintf( stderr, "Error return from Ma27_Solve: %-d\n", error );
            continue;
        }
