...
class InverseLBFGS(LinearOperator):
  """
  InverseLBFGS is a container used to store and manipulate
  ...
  :keywords:
    ...
    :scaling:   enable scaling of the 'initial matrix'. Scaling is
            ...
            Nocedal; the scaling factor is s'y/y'y.
            (default: False).
  ...
  * matvec    to compute a matrix-vector product between the current
  ...
  def __init__(self, n, npairs=5, **kwargs):
    ...
    super(InverseLBFGS, self).__init__(n, n,
  ...
  def store(self, new_s, new_y):
    ...
    is only accepted if the dot product <new_s, new_y> is over the
...
class LBFGS(InverseLBFGS):
  """
  LBFGS is similar to InverseLBFGS, except that it operates
...
class LBFGS_unrolling(InverseLBFGS):
  """
  LBFGS_unrolling is similar to LBFGS, except that matrix-vector products
  are computed by an unrolled version of the two-loop recursion.
...
class LBFGS_structured(InverseLBFGS):
  ...
  """
  def __init__(self, n, npairs=5, **kwargs):
    super(LBFGS_structured, self).__init__(self, n, npairs, **kwargs)
